...
def ensure_index(df: pd.DataFrame, excel_path: str, cache_dir: str) -> Tuple[IndexBundle, object]:
...
def hybrid_search(query: str, df: pd.DataFrame, bundle: IndexBundle, st_model=None, k: int = 5, alpha: float = 0.6) -> List[Tuple[int, float]]:
    ...
    if not query.strip():
        ...
        base = df.reset_index().index.tolist()
...
def parse_year_term_plan(text: str) -> Dict[str, Optional[str]]:
    ...
    m_year = re.search(r"ปี\s*([1-9])", text)
    if m_year:
...
def extract_interests(text: str) -> List[str]:
    ...
    stop = set(["แนะนำ","ช่วย","จัด","เทอม","ปี","วิชา","ลง","ควร","อะไร","คือ","ที่","สำหรับ","ตอน","และ","ให้","แบบ","แผน","ปกติ","สหกิจ","ก้าวหน้า","ทั้งหมด","เลือก","บังคับ"])
