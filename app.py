...
def is_topic_tool_intent(text: str) -> bool:
    # trigger when user explicitly asks for topical search
    keys = ["หัวข้อ","เกี่ยว", "เกี่ยวกับ", "คล้าย", "เรื่อง", "เกี่ยวข้อง"]
    t = text.lower()
    return any(k in t for k in keys)

def is_prereq_tool_intent(text: str) -> bool:
    # trigger phrases for "find courses that require X"
...
def main():
    ...
    with header_col2:
        ...
        <div style="text-align: center;">
            <h1 class="gradient-title">Courses Adviser</h1>
            <p class="subtitle">This is a RAG-powered Courses Adviser for CS CMU projects.</p>
        </div>
